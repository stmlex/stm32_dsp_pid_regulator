#ifndef __RTOS_LIBS_H
#define __RTOS_LIBS_H
...
/**@brief Allocate static memory and create task with stack placement macro
 * @note: if task created from another task it must be in
 * vTaskSuspendAll/xTaskResumeAll section! Or task can possibly
 * start before handle became valid
 */
#define FRTOS_TaskCreateStaticPlace(functionName, debugName, stackSize, param, prior, handle, place) \
do{ \
    static StackType_t stack [stackSize] place; \
    static StaticTask_t staticTask; \
    (handle) = xTaskCreateStatic( \
        (functionName), \
        (debugName), \
...
do{ \
    vTaskSuspendAll(); \
    FRTOS_TaskCreateStatic(functionName, debugName, stackSize, param, prior, handle); \
    if(!xTaskResumeAll()) taskYIELD(); \
}while(0)

/**@brief Allocate static memory and create task under task suspend with stack placement macro
 */
#define FRTOS_TaskCreateStaticPlaceSafe(functionName, debugName, stackSize, param, prior, handle, place) \
do{ \
    vTaskSuspendAll(); \
    FRTOS_TaskCreateStaticPlace(functionName, debugName, stackSize, param, prior, handle, place); \
    if(!xTaskResumeAll()) taskYIELD(); \
}while(0)
